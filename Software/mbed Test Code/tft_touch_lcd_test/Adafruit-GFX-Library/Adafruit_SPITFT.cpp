...
*/
/**************************************************************************/
Adafruit_SPITFT::Adafruit_SPITFT(uint16_t w, uint16_t h, PinName cs, PinName dc, PinName mosi, PinName sclk, PinName rst, PinName miso) 
  : Adafruit_GFX(w, h) {
    _cs   = cs;
    _dc   = dc;
    _rst  = rst;
    ...
    _mosi = mosi;
    _miso = miso;
    _spi = new SPI(_mosi, _miso, _sclk);
    _freq = 0;
...
/*!
    ...
    @param    w     Display width in pixels
    @param    h     Display height in pixels
    @param    spi   A pointer to an SPI hardware interface
    @param    cs    Chip select pin #
    @param    dc    Data/Command pin #
    @param    rst   Reset pin # (optional, pass -1 if unused)
*/
/**************************************************************************/
Adafruit_SPITFT::Adafruit_SPITFT(uint16_t w, uint16_t h, SPI *spi, PinName cs, PinName dc, PinName rst) 
  : Adafruit_GFX(w, h) {
    _cs   = cs;
    _dc   = dc;
    _rst  = rst;
...
void Adafruit_SPITFT::initSPI(uint32_t freq) {
    ...
    // Control Pins
    if(_cs >= 0) {
        _csPin = new DigitalOut(_cs);
        *_csPin = 1; // Deselect
    }
    _dcPin = new DigitalOut(_dc);
    *_dcPin = 0;
